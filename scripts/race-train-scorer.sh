#!/bin/bash
# scripts/race-train-scorer.sh — train the LTM importance (R) head on embodied
# outcomes and measure its effect on the revisit soft-SPL gain.
#
# WHY: the consolidator keeps only the top-k keyframes by importance
# I = αR + βU + γN; that top-k IS the fine LTM retrieval queries against. R has
# the dominant weight (α=0.4) and is, by default, a length/keyword HEURISTIC.
# This driver trains R from embodied (caption -> episode soft-SPL) data and
# wires the checkpoint in (--scorer-ckpt), then runs a 3-cell head-to-head:
#
#   S1            (memory off)         -> shared reference
#   S3 heuristic  (default R)          -> the 6x-reproduced +0.234 baseline
#   S3 trained    (learned R head)     -> the experiment
#
# The attributable scorer effect is (warm soft-SPL S3-trained) − (S3-heuristic);
# both are compared to S1 via the standard revisit analyzer (two calls, since it
# keys runs by setting and would otherwise dedupe the two S3 dirs).
#
# Train/eval split (no leak): the R head is trained on val_mini SINGLE-GOAL
# episodes (a freshly generated run, or --train-runs you point at); it is
# evaluated on the controlled-start REVISIT episodes (disjoint episode set).
#
# EXECUTE it (do NOT source):
#   bash scripts/race-train-scorer.sh --tag scorer-d1
#
# Critical invariants (mirror race-revisit.sh — each cost a re-run before):
#   * --backbone remembr + REMEMBR_STRICT=1   (no silent 'frontier'/stub run)
#   * --encoder sbert for training            (MUST match the bridge's SBERT
#                                              caption index; clip would silently
#                                              fall back to the heuristic — the
#                                              bridge now RAISES on a dim mismatch)
#   * each setting in a SEPARATE process       (LTM persists within a process)
#   * caption-quality preflight                (degenerate 'room interior'
#                                              captions => useless training data)

set -uo pipefail

# --- self-update guard: re-exec once after pull so a mid-run patch to THIS
# script can't run a half-old / half-new body (bit us on the oracle ladder). ---
if [ -z "${TRAIN_SCORER_RELAUNCHED:-}" ]; then
  REPO_ROOT="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
  cd "$REPO_ROOT" || { echo "FATAL: cannot cd to repo root"; exit 1; }
  echo "========== [0/8] git pull --ff-only (pre-exec) =========="
  git pull --ff-only || { echo "FATAL: git pull failed"; exit 1; }
  export TRAIN_SCORER_RELAUNCHED=1
  exec bash "$0" "$@"
fi
...
NWARM="3"
TAG="scorer-d1"
N_EPISODES=""          # eval: auto = one pass over the built revisit scenes
TARGET="any"
...
CKPT="models/embodied/scorer-${TAG}.pt"
...
banner "[2/8] pre-test code verify (scorer wiring + revisit + analyzer suites)"
python embodied_memory/scripts/test_scorer_wiring.py \
  || { echo "FATAL: scorer-wiring sanity suite failed — not spending on the live run."; exit 1; }
...
python embodied_memory/scripts/test_analyze_ablation.py \
  || { echo "FATAL: analyze_ablation sanity suite failed."; exit 1; }

# --- 3. build controlled-start revisit dataset (eval set) ---
...
# --- 4. training data: reuse --train-runs, else generate a val_mini single-goal run ---
if [ -n "$TRAIN_RUNS" ]; then
  ...
  # exit code conflates full-system asserts with crashes; judge by episodes written.
...
# --- 5. caption-quality preflight (degenerate captions => useless R head) ---
    ...
    for f in sorted(glob.glob(os.path.join(d, "episode_*.json"))):
        ...
        for st in ep.get("steps", []):
            ...
            if not cap:
                continue
            total += 1
            # degenerate = the all-zeros semantic-sensor fallback caption.
...
banner "[8/8a] BASELINE (heuristic R): analyze_ablation --revisit $S1 $S3H"
python embodied_memory/scripts/analyze_ablation.py --revisit "$S1" "$S3H"
banner "[8/8b] TRAINED R head: analyze_ablation --revisit $S1 $S3T"
...
banner "DONE — compare warm soft-SPL S3−S1 across the two blocks above.
  baseline (heuristic R) ~ +0.234 (6x reproduced).
  trained R head: did the warm S3−S1 delta rise, tie, or regress?
Paste BOTH Gate-A blocks (and the [5/8] caption preflight line)."
