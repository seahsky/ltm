...
## Amendment (2026-08-14): two claims in the amendment above are withdrawn

The 2026-08-08 amendment was written against `cast-1`'s scene-level sign test, which was the only test this project had at the time. Per-episode pairing (`earshot/tools/episode_diff.py`) and a null arm landed on 2026-08-12 and retired two of its sentences. They are struck here because an ADR read standalone must not carry a claim the report has withdrawn.

**"The cast is a real effect" — DEMOTED, not deleted.** Per-episode McNemar over `eps-1` → `cast-1` reads **54 gained / 40 lost over 94 discordant, exact p = 0.18**, against this amendment's scene sign test at p = 0.021. Both tests are valid; they disagree because they are sensitive to different shapes, and the cast is broad and small — 13 scenes up by +1 to +5, three down by −3 to −5 — which the sign test catches and McNemar dilutes. Two things break the tie toward caution: McNemar is anti-conservative under within-scene correlation, so 0.18 is a floor; and **+14 is below the resolution floor of 15 episodes** that the null arm established. What survives is narrower and still useful: *the cast moves 13 of 16 scenes the same way (p = 0.021), and its magnitude is unresolved at this sample size.* The ~45% rise in forward steps is untouched and remains the strongest evidence the policy does what it was built to do — though see the report: that figure is five episodes on one scene, and no aggregate exists.

**"the losses are concentrated in scenes that scored 13/20 under the coin flip" — WITHDRAWN ENTIRELY.** Three of the four supporting numbers were `cast-1`'s column read as `eps-1`'s. `funnel_diff runs/yield-2 runs/eps-1` reads 13→8, 13→7, 13→**12** and 13→**13**; two of the four barely moved under the estimator and `QaLdnwvtxbs` did not move at all. The real shape is a **broad, roughly proportional loss across 15 of the 16 scenes that moved**. Nothing about how permissive the rising test should be follows from it.

**What that does to this ADR's "next arm".** The withdrawn concentration claim was the argument that made `RISING_SIGMAS = 0` look urgent. That arm — `earshot/loosen-the-bar`, pushed with no PR on purpose — is now confounded twice: its motivating argument is gone, and it was cut against the pre-`a0f4625` stop rule, which `arrive-2` replaced with `if visual_confirm:`. It needs rebasing before its sweep isolates anything.

**The exhibit that governs all of it.** `repeat-1` re-ran `arrive-2` with a byte-identical run path and produced **+11 episodes from nothing** (exact McNemar p = 0.19). Set that beside the cast's +14 (p = 0.18) and the arrival rule's +13 (p = 0.148): **neither effect is larger than what the apparatus produces from identical bytes.** At this effect size a comparison resting on a single run of either arm is not reportable, and every comparison in this ADR is single-run. The route is repeats — three per arm, SD ÷ √3, floor ≈ 8.7 episodes.

**The 9.3-point price stands as arithmetic** (36.7% against 46.0%) but both endpoints are single runs, and `yield-2` in particular has been measured exactly once with a run-to-run SD of 5.4 episodes. Treat it as an order of magnitude, not a figure.

**The decision is still not reversed.** Nothing measured says the cast is the problem, and the exploration diagnosis it was built on survives independently: `eps-1`'s regression is the one post-reset result that clears at full power (exact McNemar **p = 0.0000**), and the plateau branch remains the live lever at ~21 episodes / 5.8 points.
