...
| **0c** | Controller actually changes behavior vs sound==goal (primary completed *after* the detour) | ⬜ next | cheap, RACE 1–2 cells |
| **E5/E6/E7** | Wire controller into `episode_runner` at the right seam (full spec §7 below; fixes C1/C2/C3 + the semantic-goal-cache snapshot); inject investigate-waypoint via proposer seam; rerank branch; report hook | ⬜ next | RACE |
