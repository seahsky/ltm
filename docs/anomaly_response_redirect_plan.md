...
## 9. Gate recalibration + every-scene mixture (#1) — the $0-Gate-0b-first plan (2026-07-01)

0c proved the forced CLAP anomaly-gate REJECTS the RIR-convolved alarm (onset only fired with `--no-anomaly-gate`). For the real eval the gate must be ON (it *is* the anomaly-vs-background discrimination), recalibrated on convolved audio; and every scene must mix background noise + the anomaly. 6-agent workflow (4 ground + design + adversarial review) → a $0-gate-first plan.

**Phase 0 — $0 Gate-0b (BUILT this turn; RUN FIRST):** `diagnose_convolved_anomaly_calib.py` + `race-convolved-anomaly-gate.sh`. Scores the EXACT live signal — `render_at_pose(anomaly)` (spatial, 2×L) **+ bg_gain·diotic(render_at_pose(bed))** — through `is_anomaly`; ACCEPT = anomaly+bed, REJECT = bed-only. Reuses `best_threshold`/`verdict`/`load_wav`. Review-driven guarantees: **M1** audible-radius-band cells (not loudest) · **M2** exact live composition, diotic bed · **M3** 2-D (delta,tau) sweep (convolution depresses s_anom — the 0c alarm→glass_break flip) · **R1/C2** `decide_gate` STOPs unless a `bg_gain` BOTH clears `onset_rms` on the post-convolution bed AND separates (EER≤0.15) — else the reject-half is vacuous · **M4** bed class ≠ a verbatim NORMAL_PROMPT · **m1** an audio-vs-audio prototype column (the pivot target, pre-measured). 9 pure-function TDD. Emits `GATE_RESULT=GO|BORDERLINE|STOP` + `RECOMMEND_DELTA/TAU/BG_GAIN`. **Run:** `nrun bash scripts/race-convolved-anomaly-gate.sh --device cuda`.

**Decision rule (pre-registered):** GO (EER≤0.15, bed clears onset) → Phases 1–3. BORDERLINE (≤0.30) → cheap ladder (lower bg_gain / hotter anomaly / drop the reverb-like normal prompt). STOP → PIVOT: (1) energy-onset + audio-prototype few-shot discriminator (text-vs-convolved-audio is CLAP's weakest regime); (2) temporally-separated benign-then-anomaly (the existing t_anom gate gives this free); (3) honest negative.
