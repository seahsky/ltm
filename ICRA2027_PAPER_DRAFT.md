...
| **Write the heard anomaly to LTM** (Step 2, store source location) | **Mechanism‑verified** (the write fires and is recalled at a distance) but **redundant with visual recall *by construction* of this harness:** the cold seed is line‑of‑sight to the source (≈0.5 m, the goal's own view‑point), so vision already maps the source and the write‑OFF baseline already routes there — the audio write is a duplicate at the same location, whose best case is therefore a tie (write‑ON vs write‑OFF paired soft‑SPL ≈ 0 once an over‑fire confound is damped). This is a designed boundary probe, not an unfinished mechanism: it is the sixth confirming instance of the unifying claim below. |
