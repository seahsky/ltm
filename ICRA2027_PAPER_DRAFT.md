...
| **GOAT‑Bench** [Khanna et al., CVPR 2024] | Benchmark (tests explicit/implicit memory) | Lifelong **multi‑goal** (5–10 sub‑goals/ep), HM3D | Within‑episode memory reuse | We test **cross‑visit** warm/cold recall with paired bootstrap CIs; **open: run a GOAT slice or justify the revisit harness (§7)** |
| **TrajRAG** [CVPR 2026] ⚠ | Hierarchical coarse→fine retrieval over past trajectories | Zero‑shot ObjectNav, HM3D/MP3D | Yes (lifelong trajectory accumulation) | Closest method; we add consolidation/importance + the **boundary+bottleneck diagnosis** (it reports *that* it helps, not *when* it stops) |
...
| **SSMG‑Nav** [2026] ⚠ | Persistent consolidating semantic skeleton graph | Lifelong multimodal ObjectNav, GOAT (0.341 SPL vs VLFM 0.159) | Yes | SOTA bar; we are a **memory‑delta** study on a weaker backbone, not an absolute‑SOTA claim |
...
| **VLFM** [Yokoyama & Ha, ICRA 2024] | BLIP‑2/CLIP image‑text value map (no LTM) | Zero‑shot ObjectNav, HM3D | No | Near‑identical category‑cosine core; **open: run as external frontier‑only baseline (closest to our S1/S2, §7)** |
| **HomeEmergency** [Yan et al., 2025] | Probabilistic dynamic scene graph + Bayesian audio‑direction fusion | **Map home → hear anomaly → navigate → VLM‑verify**, ThreeDWorld | No (no cross‑visit consolidated LTM) | The nearest audio task; we differentiate on **persistent consolidated cross‑visit LTM + warm/cold recall** (they have none) and HM3D+SoundSpaces vs TDW |
| **Habitat‑Echo / STMM** [2026] ⚠ | Sound‑triggered response, binaural RIRs | Sound‑triggered mobile manipulation, Habitat | No (no persistent memory) | Concurrent; we differentiate on **persistent LTM + lifelong warm/cold recall** (not manipulation) |
...
| **rliable** [Agarwal et al., NeurIPS 2021] | Statistical‑rigor reference (not a system) | Small‑n RL/embodied eval | N/A | We frame our paired‑bootstrap + 90% CI design as in‑spirit; **open: report effective independent n (≈2–3 datasets) + a performance profile (§7)** |
