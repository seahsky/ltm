...
- **How the `ss2` env is pinned, and how it is recreated.**
  Surfaced by ticket 13 while still in progress, and it stands whatever that ticket's box run says.
  The env that the entire destination runs on has **no lockfile**: `oneenv_gate.sh` installs from version *ranges*, so every re-run re-resolves against whatever PyPI serves that day. That is not a hypothetical — it is exactly how the env broke, with `transformers` drifting up to 4.57.6 against a frozen `torch==2.0.1` and disabling its own torch backend.
  Ticket 13 addresses the instance (move the pin) and the detection (the gate now fails loudly on a non-working pair), but not the class: the clean room's env is currently reproduced by re-running a resolver, not by restoring a pinned state. Whether that matters is a real question rather than an obvious yes — a `pip freeze` of a half-built habitat-sim tree is its own maintenance burden, and the audio build is compiled from source anyway, so the lockfile would only cover part of the surface.
  Waits on nothing in particular; sharpens once ticket 13 has a GREEN and the working set of versions is actually known. Related: ticket 14 is extracting the box runbook, which is where a pinned env would naturally live.
