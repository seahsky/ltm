...
- **How far the clean room is willing to fork habitat-sim.**
  ...
  **Settled by 06 (2026-08-01): the patch is NOT budget-gated.** One render at the cheap preset is 27.2 ms, so three sequential renders is ~82 ms/step (~41 s/episode) — inside budget without any patch. The band in which the patch had to be taken for cost reasons is empty, so this entry no longer waits on a measurement at all: **ticket 09 decides the multi-source patch purely on onset provenance**, and the fork question reduces to whether that one property is worth owning a habitat-sim fork for.
